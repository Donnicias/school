...
*/
	...
	function call_feetype_amount() {
		$feetype = $this->input->post('feetype');
		if($feetype !="") {
			$feetypeaa = $this->feetype_m->allfeetype($feetype);;
			foreach ($feetypeaa as $feetype) {
				echo '
				<input type="text" class="form-control" id="amount" name="amount" value="'.$feetype->amount.'" >
				';
			}
		} else {
			echo "<input type=\"text\" value=\" nothing found\" />";
		}
	}
