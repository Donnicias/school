...
*/
	...
	public function index() {
		...
		if($usertype == "Admin" || $usertype == "Librarian") {
			$ulID = htmlentities(mysqli_real_escape_string($this->db->conn_id,$this->uri->segment(3)));
			$lID = htmlentities(mysqli_real_escape_string($this->db->conn_id,$this->input->post("lid")));
			if($lID != "" || !empty($lID)) {
		...
		if($usertype == "Admin" || $usertype == "Librarian") {
			if($_POST) {
				...
				if ($this->form_validation->run() == FALSE) {
					...
					$this->book_m->update_book(array("status" => 1), $this->input->post("bookID"));
	...
	public function edit() {
		...
		if($usertype == "Admin" || $usertype == "Librarian") {
			$id = htmlentities(mysqli_real_escape_string($this->db->conn_id,$this->uri->segment(3)));
			$lID = htmlentities(mysqli_real_escape_string($this->db->conn_id,$this->uri->segment(4)));
			if((int)$id && $lID) {
	...
	public function fine() {
		...
		if($usertype == "Admin" || $usertype == "Librarian") {
			if($_POST) {
				$day = htmlentities(mysqli_real_escape_string($this->db->conn_id,$this->input->post("day")));
				$month = htmlentities(mysqli_real_escape_string($this->db->conn_id,$this->input->post("month")));
				$year = htmlentities(mysqli_real_escape_string($this->db->conn_id,$this->input->post("year")));
	...
	public function print_preview() {
		...
		if($usertype == "Admin" || $usertype == "Librarian") {
			...
			$day = htmlentities(mysqli_real_escape_string($this->db->conn_id,$this->uri->segment(5)));
			$month = htmlentities(mysqli_real_escape_string($this->db->conn_id,$this->uri->segment(4)));
			$year = htmlentities(mysqli_real_escape_string($this->db->conn_id,$this->uri->segment(3)));
		...
		if($usertype == "Admin" || $usertype == "Librarian") {
			...
			if((int)$id && $lID) {
				...
				if($issue) {
					...
					$this->book_m->update_book(array("status" => 0), $dbGet_bookID);
	...
	public function unique_quantity() {
		$id = htmlentities(mysqli_real_escape_string($this->db->conn_id,$this->uri->segment(3)));
		$bookID = $this->input->post("bookID");
		$isbn = $this->input->post("serial_no");
		$author = $this->input->post("author");

		if($id) {
			if((int)$bookID) {
				$bookandauthor = $this->issue_m->get_single_issue(array("serial_no" => $isbn));

				if(count($bookandauthor)) {
					return TRUE;
				}
			} else {
				$this->form_validation->set_message("unique_quantity", "%s is not available.");
				return FALSE;
			}

		} else {
			if((int)$bookID) {
				$bookandauthor = $this->book_m->get_single_book(array("ISBN" => $isbn));
				$bookborrower = $this->issue_m->get_single_issue(array("serial_no" => $isbn));
				if(count($bookandauthor)) {
					if($bookandauthor->status == 1) {
						$this->form_validation->set_message("unique_quantity", "%s is not available. It was borrowed on ".$bookborrower->issue_date." and hasn't been returned.");
						return FALSE;
					}else if($bookandauthor->status == 99){
						$this->form_validation->set_message("unique_quantity", "%s is lost. Not yet found.");
						return FALSE;
					}else{
						return TRUE;
					}
				...
				$this->form_validation->set_message("unique_quantity", "%s Wrong book id 2.");
				return FALSE;
			}
		}	
	}
	...
	function bookIDcall() {
		...
		if($bookID) {
			...
			$json = array("author" => $author, "subject_code" => $subject_code, "isbn" => $isbn);
