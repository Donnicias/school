		...
		if($usertype == "Admin" || $usertype == "Teacher") {
			...
			if ((int)$classID && (int)$examID && (int)$studentID && (int)$position) {
				...
				if($student) {
				    ...
				    $oposition=$this->ranking_m->get_overall_position($examID,$classID,$student->roll);

				    
				    $cposition=$this->ranking_m->get_class_position($examID,$classID,$student->sectionID,$student->roll);
				    //print_r($cposition);
				    $transcriptss= $this->reportcardss_m->get_order_by_transcripts_with_highest_mark($classID,$studentID,$examID);
			...
			if((int)$classID && (int)$year) {
			    ...
			    if(count($student)>0){
			    	foreach ($student as $key) {
					    ...
					    $this->pdf->SetFont('','');
					    $this->pdf->Cell(0,5,"Closing date: 07/04/2017 ",0,0,'L');
					    $this->pdf->Cell(0,5,"Opening Date: 02/05/2017",0,1,'R');
					    $this->pdf->ln(1);
	...
	function FancyTable($header,$student,$classID,$transcriptss,$cposition,$oposition,$examinations,$examIDs,$year,$sectionID,$roll)
				{
					...
					if ($classID==13 || $classID==14) {
						...
						if ($totalstudents>0) {
							foreach ($transcriptss as $transcripts) {
						        ...
						        foreach ($examinations as $value) {
						        	...
						        	if (count($subject_score) !=0) {
							        	foreach ($subject_score as $keyvalue) {
						        			...
						        			$this->pdf->Cell($w[3],6,'n/a','LR',0,'L',$fill);
						        	}
						        }
						        if (sizeof($examinations)>2) {
					      	...
					      	$this->pdf->Cell(62,6,strtoupper("End term results summary (S.E. ".$oposition['se'].')'),0,0,'C');
							$this->pdf->Cell(53,6,"T.Points: ".$oposition['tp'],0,0,'R');
							$this->pdf->Cell(20,6,"M.Grade: ".$ssgrade,0,0,'R');
							$this->pdf->Cell(29,6,"Position: ".$oposition['position'].'/'.$oposition['totalstudents'],0,0,'R');
							//$this->pdf->Cell(29,6,"C.Position: ".$cposition['position'].'/'.$cposition['totalstudents'],0,1,'R');
						}else{
						...
						if ($classID==13 || $classID==14) {
							$this->pdf->Cell(35,6,"Pos: ".$oposition['position'].'/'.$oposition['totalstudents'],0,1,'L');
						}else{
				    ...
				    $this->pdf->Cell(62,5,$kcpe_data['former_primary'],1,0,'L');
				    ...
				    foreach ($terms as $value) {
				    	$oposs=$this->ranking_m->get_overall_position_end_term($examIDs,$classID,$roll,$year,$value->term_name);
				    		if ($classID !=13 && $classID !=14) {
					    		...
					    		$kygrades = $this->grade_m->get_grade(array("classesID"=>1,"subjectID"=>1));
					    		if(count($kygrades)) {
					    			if ($markkk !=0) {
					    				foreach ($kygrades as $grade) {
						                      if($grade->gradefrom <= round($markkk/11,0) && $grade->gradeupto >= round($markkk/11,0)) {
				    			...
				    			$ttp = $cpos['avg_points'];
					    		$kddygrades = $this->grade_m->get_grade(array("classesID"=>1,"subjectID"=>1));
					    		if(count($kddygrades)) {
					    			if (!empty($ttp)) {
					    				foreach ($kddygrades as $grade) {
						                      if($grade->point <= $ttp && $grade->point >= $ttp) {
						                           	$this->pdf->Cell(30,5,'M.Grade: '.$grade->grade,1,0,'L');
						                      }
						                }
					    			}else{
