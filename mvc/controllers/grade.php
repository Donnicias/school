<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');

class Grade extends Admin_Controller {
/*
| -----------------------------------------------------
| PRODUCT NAME: 	NIFTECS SCHOOL MANAGEMENT SYSTEM
| -----------------------------------------------------
| AUTHOR:			NIFTECS TEAM
| -----------------------------------------------------
| EMAIL:			info@niftecs.com
| -----------------------------------------------------
| COPYRIGHT:		RESERVED BY NIFTECS SOFTWARES
| -----------------------------------------------------
| WEBSITE:			http://niftecs.com
| -----------------------------------------------------
*/
	function __construct() {
		parent::__construct();
		$this->load->model("grade_m");
		$this->load->model("classes_m");
		$this->load->model("subject_m");
		$this->load->model("student_m");
		$language = $this->session->userdata('lang');
		$this->lang->load('grade', $language);	
	}

	public function index() {
		$usertype = $this->session->userdata("usertype");
		if($usertype == "Admin") {
			$classID = htmlentities(mysqli_real_escape_string($this->db->conn_id,$this->uri->segment(3)));
			if((int)$classID){
				$array=array(
					"grade.classesID"=>$classID
					);
				$this->data['grades'] = $this->grade_m->get_order_by_grade($array);
				$this->data["subview"] = "grade/index";
				$this->load->view('_layout_main', $this->data);
			}else{
				$this->data['classes'] = $this->student_m->get_classes();
				$this->data["subview"] = "grade/search";
				$this->load->view('_layout_main', $this->data);
			}
		} else {
			$this->data["subview"] = "error";
			$this->load->view('_layout_main', $this->data);
		}
	}

	protected function rules() {
		$rules = array(
				array(
					'field' => 'grade', 
					'label' => $this->lang->line("grade_name"), 
					'rules' => 'trim|required|xss_clean|max_length[60]|callback_unique_grade'
				), 
				array(
					'field' => 'point', 
					'label' => $this->lang->line("grade_point"),
					'rules' => 'trim|required|numeric|max_length[11]|xss_clean|callback_unique_point'
				), 
				array(
					'field' => 'classesID', 
					'label' => $this->lang->line("grade_classes"),
					'rules' => 'trim|required|numeric|max_length[11]|xss_clean'
				), 
				array(
					'field' => 'subjectID', 
					'label' => $this->lang->line("grade_subject"),
					'rules' => 'trim|required|numeric|max_length[11]|xss_clean'
				), 
				array(
					'field' => 'gradefrom', 
					'label' => $this->lang->line("grade_gradefrom"),
					'rules' => 'trim|required|numeric|max_length[11]|xss_clean|callback_unique_gradefrom'
				),
				array(
					'field' => 'gradeupto', 
					'label' => $this->lang->line("grade_gradeupto"),
					'rules' => 'trim|required|numeric|max_length[11]|xss_clean|callback_unique_gradeupto'
				), 
				array(
					'field' => 'note', 
					'label' => $this->lang->line("grade_note"), 
					'rules' => 'trim|max_length[200]|xss_clean'
				)
			);
		return $rules;
	}

	public function add() {
		$usertype = $this->session->userdata("usertype");
		if($usertype == "Admin") {
				$this->data['set_classes'] = 0;
				$this->data['set_subject'] = 0;
				$this->data['classes'] = $this->classes_m->get_classes();
				$this->data['subjects'] = 0;
			if($_POST) {
				$rules = $this->rules();
				$this->form_validation->set_rules($rules);
				if ($this->form_validation->run() == FALSE) {
					$this->data["subview"] = "grade/add";
					$this->load->view('_layout_main', $this->data);			
				} else {
					$array = array(
						"classesID" => $this->input->post("classesID"),
						"subjectID" => $this->input->post("subjectID"),
						"grade" => $this->input->post("grade"),
						"point" => $this->input->post("point"),
						"gradefrom" => $this->input->post("gradefrom"),
						"gradeupto" => $this->input->post("gradeupto"),
						"note" => $this->input->post("note")
					);

					$this->grade_m->insert_grade($array);
					$this->session->set_flashdata('success', $this->lang->line('menu_success'));
					redirect(base_url("grade/index"));
				}
			} else {
				$this->data["subview"] = "grade/add";
				$this->load->view('_layout_main', $this->data);
			}
		} else {
			$this->data["subview"] = "error";
			$this->load->view('_layout_main', $this->data);
		}
	}

	public function edit() {
		$usertype = $this->session->userdata("usertype");
		if($usertype == "Admin") {
			$this->data['set_classes'] = 0;
				$this->data['set_subject'] = 0;
				$this->data['classes'] = $this->classes_m->get_classes();
				$this->data['subjects'] = 0;
			$id = htmlentities(mysqli_real_escape_string($this->db->conn_id,$this->uri->segment(3)));
			if((int)$id) {
				$this->data['grade'] = $this->grade_m->get_grade(array("gradeID"=>$id));
				if($this->data['grade']) {
					if($_POST) {
						$rules = $this->rules();
						$this->form_validation->set_rules($rules);
						if ($this->form_validation->run() == FALSE) {
							$this->data["subview"] = "grade/edit";
							$this->load->view('_layout_main', $this->data);			
						} else {
							$array = array(
								"grade" => $this->input->post("grade"),
								"point" => $this->input->post("point"),
								"gradefrom" => $this->input->post("gradefrom"),
								"gradeupto" => $this->input->post("gradeupto"),
								"note" => $this->input->post("note")
							);

							$this->grade_m->update_grade($array, $id);
							$this->session->set_flashdata('success', $this->lang->line('menu_success'));
							redirect(base_url("grade/index"));
						}
					} else {
						$this->data["subview"] = "grade/edit";
						$this->load->view('_layout_main', $this->data);
					}
				} else {
					$this->data["subview"] = "error";
					$this->load->view('_layout_main', $this->data);
				}
			} else {
				$this->data["subview"] = "error";
				$this->load->view('_layout_main', $this->data);
			}
		} else {
			$this->data["subview"] = "error";
			$this->load->view('_layout_main', $this->data);
		}
	}

	public function delete() {
		$usertype = $this->session->userdata("usertype");
		if($usertype == "Admin") {
			$id = htmlentities(mysqli_real_escape_string($this->db->conn_id,$this->uri->segment(3)));
			if((int)$id) {
				$this->grade_m->delete_grade($id);
				$this->session->set_flashdata('success', $this->lang->line('menu_success'));
				redirect(base_url("grade/index"));
			} else {
				redirect(base_url("grade/index"));
			}	
		} else {
			$this->data["subview"] = "error";
			$this->load->view('_layout_main', $this->data);
		}
	}

	public function unique_grade() {
		$id = htmlentities(mysqli_real_escape_string($this->db->conn_id,$this->uri->segment(3)));
		if((int)$id) {
			$grade = $this->grade_m->get_order_by_grade(array("grade.grade" => $this->input->post("grade"), "grade.gradeID !=" => $id,"grade.classesID" => $this->input->post("classesID"),"grade.subjectID" => $this->input->post("subjectID")));
			if(count($grade)) {
				$this->form_validation->set_message("unique_grade", "%s already exists");
				return FALSE;
			}
			return TRUE;
		} else {
			$grade = $this->grade_m->get_order_by_grade(array("grade.grade" => $this->input->post("grade"),"grade.classesID" => $this->input->post("classesID"),"grade.subjectID" => $this->input->post("subjectID")));

			if(count($grade)) {
				$this->form_validation->set_message("unique_grade", "%s already exists");
				return FALSE;
			}
			return TRUE;
		}	
	}

	public function unique_point() {
		$id = htmlentities(mysqli_real_escape_string($this->db->conn_id,$this->uri->segment(3)));
		if((int)$id) {
			$grade = $this->grade_m->get_order_by_grade(array("grade.point" => $this->input->post("point"), "grade.gradeID !=" => $id,"grade.classesID" => $this->input->post("classesID"),"grade.subjectID" => $this->input->post("subjectID")));
			if(count($grade)) {
				$this->form_validation->set_message("unique_point", "%s already exists");
				return FALSE;
			}
			return TRUE;
		} else {
			$grade = $this->grade_m->get_order_by_grade(array("grade.point" => $this->input->post("point"),"grade.classesID" => $this->input->post("classesID"),"grade.subjectID" => $this->input->post("subjectID")));

			if(count($grade)) {
				$this->form_validation->set_message("unique_point", "%s already exists");
				return FALSE;
			}
			return TRUE;
		}	
	}

	public function unique_gradefrom() {
		$id = htmlentities(mysqli_real_escape_string($this->db->conn_id,$this->uri->segment(3)));
		if((int)$id) {
			$grade = $this->grade_m->get_order_by_grade(array("grade.gradefrom" => $this->input->post("gradefrom"), "grade.gradeID !=" => $id));
			if(count($grade)) {
				$this->form_validation->set_message("unique_gradefrom", "%s already exists");
				return FALSE;
			}
			return TRUE;
		} else {
			$grade = $this->grade_m->get_order_by_grade(array("grade.gradefrom" => $this->input->post("gradefrom"),"grade.classesID" => $this->input->post("classesID"),"grade.subjectID" => $this->input->post("subjectID"),));

			if(count($grade)) {
				$this->form_validation->set_message("unique_gradefrom", "%s already exists");
				return FALSE;
			}
			return TRUE;
		}	
	}

	public function unique_gradeupto() {
		$id = htmlentities(mysqli_real_escape_string($this->db->conn_id,$this->uri->segment(3)));
		if((int)$id) {
			$grade = $this->grade_m->get_order_by_grade(array("grade.gradeupto" => $this->input->post("gradeupto"),"grade.classesID" => $this->input->post("classesID"),"grade.subjectID" => $this->input->post("subjectID")));
			if(count($grade)) {
				$this->form_validation->set_message("unique_gradeupto", "%s already exists".$this->input->post("classesID"));
				return FALSE;
			}
			return TRUE;
		} else {
			$grade = $this->grade_m->get_order_by_grade(array("grade.gradeupto" => $this->input->post("gradeupto"),"grade.classesID" => $this->input->post("classesID"),"grade.subjectID" => $this->input->post("subjectID")));

			if(count($grade)) {
				$this->form_validation->set_message("unique_gradeupto", "%s already exists".$this->input->post("classesID"));
				return FALSE;
			}
			return TRUE;
		}	
	}
	function grade_list() {
		$classID = $this->input->post('classesID');
		if((int)$classID) {
			$string = base_url("grade/index/$classID");
			echo $string;
		} else {
			redirect(base_url("grade/index"));
		}
	}
}

/* End of file grade.php */
/* Location: .//D/xampp/htdocs/school/mvc/controllers/grade.php */