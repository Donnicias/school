...
*/
	...
	protected function rules() {
		$rules = array(
			array(
				'field' => 'doa',
				'label' => $this->lang->line("student_doa"),
			...
			array(
				'field' => 'former_primary',
				'label' => $this->lang->line("student_former_primary"),
				'rules' => 'trim|required|max_length[100]|min_length[4]|xss_clean'
			),
			array(
				'field' => 'kcpe_year',
				'label' => $this->lang->line("student_kcpe_year"),
				'rules' => 'trim|numeric|required|max_length[4]|xss_clean'
			),
			array(
				'field' => 'kcpe_mark',
				'label' => $this->lang->line("student_kcpe_mark"),
				'rules' => 'trim|numeric|required|max_length[3]|xss_clean'
			),
			array(
				'field' => 'kcpe_index',
				'label' => $this->lang->line("student_kcpe_idex"),
				'rules' => 'trim|numeric|required|callback_unique_kcpe_index|max_length[12]|xss_clean'
			),
			array(
				'field' => 'parent_status',
				'label' => $this->lang->line("student_parent_status"),
				'rules' => 'trim|required|max_length[20]|min_length[2]|xss_clean'
			),
			array(
				'field' => 'health',
				'label' => $this->lang->line("student_health"),
				'rules' => 'trim|required|max_length[100]|min_length[2]|xss_clean'
			),
			array(
				'field' => 'guargianID',
		...
		if($usertype == "Admin") {
			...
			if($_POST) {
				...
				if ($this->form_validation->run() == FALSE) {
					...
					$array = array();
					$array["doa"] = date("Y-m-d", strtotime($this->input->post("doa")));
					...
					$array["former_primary"] = $this->input->post("former_primary");
					$array["kcpe_year"] = $this->input->post("kcpe_year");
					$array["kcpe_mark"] = $this->input->post("kcpe_mark");
					$array["kcpe_index"] = $this->input->post("kcpe_index");
					$array["parent_status"] = $this->input->post("parent_status");
		...
		if($usertype == "Admin") {
			...
			if((int)$id && (int)$url) {
				...
				if($this->data['student']) {
					if($_POST) {
						$rules = $this->rules();
						unset($rules[18],$rules[19], $rules[20]);
						$this->form_validation->set_rules($rules);
						if ($this->form_validation->run() == FALSE) {
	...
	public function unique_kcpe_index() {
		$id = htmlentities(mysqli_real_escape_string($this->db->conn_id,$this->uri->segment(3)));
		if((int)$id) {
			$student = $this->student_m->get_order_by_roll(array("roll" => $this->input->post("kcpe_index"), "studentID !=" => $id, "classesID" => $this->input->post('classesID')));
			if(count($student)) {
				$this->form_validation->set_message("unique_kcpe_index", "%s already exists");
				return FALSE;
			}
			return TRUE;
		} else {
			$student = $this->student_m->get_order_by_roll(array("roll" => $this->input->post("kcpe_index"), "classesID" => $this->input->post('classesID')));
	...
	function datacall() {
		$guargianID = $this->input->post('id');
		if((int)$guargianID) {
			$returneddata = $this->parentes_m->get_parent_details($guargianID);
			foreach ($returneddata as  $value) {
				echo '<input type="text" class="form-control" id="address" name="address" value="'.$value['address'].'" >';
			}
		}
	}

	function datacallemail() {
		$guargianID = $this->input->post('id');
		if((int)$guargianID) {
			$returneddata = $this->parentes_m->get_parent_details($guargianID);
			foreach ($returneddata as  $value) {
				echo '<input type="text" class="form-control" id="email" name="email" value="'.$value['email'].'" >';
			}
		}
	}

	function datacallphone() {
		$guargianID = $this->input->post('id');
		if((int)$guargianID) {
			$returneddata = $this->parentes_m->get_parent_details($guargianID);
			foreach ($returneddata as  $value) {
				echo '<input type="text" class="form-control" id="phone" name="phone" value="'.$value['phone'].'" >';
			}
		}
	}
