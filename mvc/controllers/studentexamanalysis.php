...
class Studentexamanalysis extends Admin_Controller {
/*
| -----------------------------------------------------
...
*/
	function __construct() {
		...
		$this->load->model("studentexamanalysis_m");
		...
		$this->lang->load('studentexamanalysis', $language);
	}
	...
	public function index() {
		...
		if($usertype == "Admin" || $usertype == "Teacher") {
			...
			if((int)$id) {
				...
				$this->data["subview"] = "studentexamanalysis/index";
				$this->load->view('_layout_main', $this->data);
			} else {
				$this->data['classes'] = $this->student_m->get_classes();
				$this->data["subview"] = "studentexamanalysis/search";
				$this->load->view('_layout_main', $this->data);
			}
			...
			if((int)$id) {
				...
				if(count($checkstudent)) {
					...
					if($this->data["student"] && $this->data["classes"]) {
						...
						$this->data["marks"] = $this->studentexamanalysis_m->get_order_by_mark(array("studentID" => $student->studentID, "classesID" => $student->classesID));
						$this->data["section"] = $this->section_m->get_section($this->data['student']->sectionID);

						$this->data["subview"] = "studentexamanalysis/index_parent";
						$this->load->view('_layout_main', $this->data);
					} else {
				...
				$this->data["subview"] = "studentexamanalysis/search_parent";
				$this->load->view('_layout_main', $this->data);
			}
		...
		foreach ($students as $student) {
			foreach ($ex_array as $key => $mark) {
				if($key == $student->studentID) {
					...
					$this->studentexamanalysis_m->update_mark_classes($array, array("examID" => $examID, "classesID" => $classesID, "subjectID" => $subjectID, "studentID" => $student->studentID));
					break;
				}
			}
		}
		...
		if($usertype == "Admin" || $usertype == "Teacher") {
			...
			if ((int)$id && (int)$url) {
				...
				if($this->data["student"] && $this->data["classes"]) {
					...
					$this->data["exams"] = $this->exam_m->get_exam();
					$this->data["grades"] = $this->grade_m->get_grade();
					$this->data["marks"] = $this->studentexamanalysis_m->get_order_by_mark_with_highest_mark($url,$id);
					$this->data["section"] = $this->section_m->get_section($this->data['student']->sectionID);
					...
					$this->data["subview"] = "studentexamanalysis/view";
					$this->load->view('_layout_main', $this->data);
				} else {
					$this->data["subview"] = "error";
					$this->load->view('_layout_main', $this->data);
		...
		if($usertype == "Admin" || $usertype == "Teacher") {
			...
			if ((int)$id && (int)$url) {
				...
				if($this->data["student"] && $this->data["classes"]) {
					...
					$this->data["exams"] = $this->exam_m->get_exam();
					$this->data["grades"] = $this->grade_m->get_grade();
					$this->data["marks"] = $this->studentexamanalysis_m->get_order_by_mark(array("studentID" =>$id, "classesID" => $url));
					$this->data["section"] = $this->section_m->get_section($this->data['student']->sectionID);
					...
					$html = $this->load->view('studentexamanalysis/print_preview', $this->data, true);
		...
		if($usertype == "Admin" || $usertype == "Teacher") {
			...
			if($_POST) {
				...
				if ($this->form_validation->run() == FALSE) {
					$this->data["subview"] = "studentexamanalysis/add";
					$this->load->view('_layout_main', $this->data);
				} else {
					$examID = $this->input->post('examID');
					$classesID = $this->input->post('classesID');
					...
					$students = $this->student_m->get_order_by_student(array("classesID" => $classesID));
						...
						if(count($students)) {
							foreach ($students as $student) {
								...
								$in_student = $this->studentexamanalysis_m->get_order_by_mark(array("examID" => $examID, "classesID" => $classesID, "subjectID" => $subjectID, "studentID" => $studentID));
								if(!count($in_student)) {
									$array = array(
										"examID" => $examID,
										"exam" => $exam->exam,
									...
									$this->studentexamanalysis_m->insert_mark($array);
								}
							}
							$this->data['students'] = $students;
							$all_student = $this->studentexamanalysis_m->get_order_by_mark(array("examID" => $examID, "classesID" => $classesID, "subjectID" => $subjectID));
	...
	function mark_list() {
		...
		if((int)$classID) {
			$string = base_url("studentexamanalysis/index/$classID");
			echo $string;
		} else {
			redirect(base_url("studentexamanalysis/index"));
		}
	}
	...
	function student_list() {
		...
		if((int)$studentID) {
			$string = base_url("studentexamanalysis/index/$studentID");
			echo $string;
		} else {
			redirect(base_url("studentexamanalysis/index"));
		}
