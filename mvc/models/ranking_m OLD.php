...
class Ranking_m extends MY_Model {
	...
	function get_sstudent($classesID,$sectionID,$examID,$year,$term) {
		$this->db->select('student.kcpe_mark,student.name,student.roll, student.studentID, avg(score) as avg, sum(score) as studenttotal')->from('processed_mark')->where(array('processed_mark.examID'=>$examID,'processed_mark.term_name'=>$term,'processed_mark.year'=>$year,'processed_mark.classesID'=>$classesID,'student.sectionID'=>$sectionID))->group_by('processed_mark.studentID')->order_by('studenttotal desc');
	...
	function get_sstudent_end_term($classesID,$sectionID,$examID,$year,$term) {
		$this->db->select('student.kcpe_mark,student.name,student.roll, student.studentID, avg((mark.mark/out_of)*100) as avg, sum((mark.mark/out_of)*100) as studenttotal')->from('mark');
		$this->db->where(array('mark.classesID'=>$classesID,'student.sectionID'=>$sectionID,'mark.year'=>$year,'term_name'=>$term));
		$this->db->where('mark.examID IN ('.$examID.')');
		$this->db->group_by('mark.studentID');
		$this->db->order_by('studenttotal desc');
		$this->db->join('student', 'student.studentID = mark.studentID', 'LEFT');
		$query = $this->db->get();
		return $query->result();
	}

	function get_sstudents($classesID,$sectionID,$examID) {
		$this->db->select('student.studentID,student.kcpe_mark,student.name,student.roll, student.studentID, f.total_points as tp,f.overall_grade as grade, f.subject_entry as se')->from('mark')->where(array('mark.examID'=>$examID,'mark.classesID'=>$classesID,'student.sectionID'=>$sectionID,'f.examID'=>$examID))->group_by('mark.studentID');
		$this->db->join('student', 'student.studentID = mark.studentID', 'LEFT');
	...
	function get_sstudents_end_term($classesID,$sectionID,$examID,$year,$term) {
		$this->db->select('student.studentID,student.kcpe_mark,student.name,student.roll, student.studentID, avg(f.total_points) as tp,f.overall_grade as grade, f.subject_entry as se')->from('mark');
		$this->db->where(array('mark.classesID'=>$classesID,'student.sectionID'=>$sectionID,'mark.year'=>$year,'mark.term_name'=>$term,'f.term'=>$term,'mark.year'=>$year,'f.year'=>$year));
		$this->db->where('mark.examID IN ('.$examID.')');
		$this->db->where('f.examID IN ('.$examID.')');
		$this->db->group_by('mark.studentID');
		$this->db->join('student', 'student.studentID = mark.studentID', 'LEFT');
		$this->db->join('form3and4averages f', 'f.roll = mark.studentID', 'LEFT');
		$this->db->order_by('tp desc');
	...
	function gets_sstudents($classesID,$examID,$year,$term_name) {
		$this->db->select('student.sectionID,student.kcpe_mark,student.photo,student.name,student.roll, student.studentID, f.total_points as tp,f.overall_grade as grade, f.subject_entry as se')->from('mark')->where(array('mark.examID'=>$examID,'mark.classesID'=>$classesID,'f.examID'=>$examID))->group_by('mark.studentID');
		$this->db->join('student', 'student.studentID = mark.studentID', 'LEFT');
		$this->db->join('form3and4averages f', 'f.roll = mark.studentID', 'LEFT');
		$this->db->order_by('tp desc');
		$query = $this->db->get();
	...
	function gets_sstudents_end_term($classesID,$examID,$year,$term) {
		$this->db->select('student.sectionID,student.kcpe_mark,student.photo,student.name,student.roll, student.studentID, avg(f.total_points) as tp,f.overall_grade as grade, f.subject_entry as se')->from('mark');
...
function sciences_rIDs($studentID,$examID,$classesID){
	...
	function get_subject_mark($studentID,$examID,$subjectID,$year,$term_name) {
		$query = $this->db->get_where('processed_mark', array('studentID' => $studentID,'examID' => $examID,'subjectID' => $subjectID,'year' => $year,'term_name' => $term_name));
		return $query->result();
	}
	...
	function get_subjects_marks_end_term($studentID,$examID,$subjectID,$year,$term) {
		$this->db->select('(mark/out_of)*100 as mark')->from("mark");
		$this->db->where(array('studentID' => $studentID,'subjectID' => $subjectID,'mark !=' => 'n','year'=>$year,'term_name'=>$term,'examID'=>$examID));
		$query = $this->db->get();
		return $query->result();
	}

	function get_total_subject_count($studentID,$examID,$classesID) {
		$this->db->select('count(*) as sno')->from('student')->where(array('student.roll'=>$studentID,'mark.examID'=>$examID,'mark.classesID'=>$classesID,'mark.mark !='=>'n'));
	...
	function get_student_total($studentID,$examID,$classesID) {
		$this->db->select_sum('mark')->from('mark')->where(array('examID'=>$examID,'classesID'=>$classesID,'studentID'=>$studentID));
		$query = $this->db->get();
		return $query->result();
	}

	function get_subject_total($examID,$subjectID,$classesID) {
		$query=$this->db->select('sum((mark/out_of)*100) as mark')->from('mark')->where(array('examID'=>$examID,'classesID'=>$classesID,'subjectID'=>$subjectID,'mark !='=>'n'));
		$query = $this->db->get();
		return $query->result();
	}
	...
	function get_ssubject_total($examID,$subjectID,$classesID,$sectionID) {
		$this->db->select('sum((mark.mark/mark.out_of)*100) as mark')->from('mark');
		$this->db->join('student', 'student.studentID = mark.studentID', 'LEFT');
		$this->db->where(array("student.sectionID"=>$sectionID,"mark.examID"=>$examID,"mark.classesID"=>$classesID,"mark.subjectID"=>$subjectID,"mark.mark !="=>"n"));
		$query=$this->db->get();
		return $query->result();
	}
	...
	function get_ssubject_total_end_term($examID,$subjectID,$classesID,$sectionID,$term,$year) {
		$this->db->select('sum((mark.mark/mark.out_of)*100) as mark')->from('mark');
		$this->db->join('student', 'student.studentID = mark.studentID', 'LEFT');
		$this->db->where(array("student.sectionID"=>$sectionID,"mark.classesID"=>$classesID,"mark.subjectID"=>$subjectID,"mark.mark !="=>"n",'mark.term_name'=>$term,'mark.year'=>$year));
		$this->db->where("mark.examID IN (".$examID.')');
		$query=$this->db->get();
		return $query->result();
	}
	...
	function get_subject_average_end_term($examID,$subjectID,$classesID,$year) {
	...
	function get_ssubject_average($examID,$subjectID,$classesID,$sectionID) {
		$this->db->select('avg((mark.mark/mark.out_of)*100) as mark')->from('mark')->where(array("mark.examID"=>$examID,"mark.classesID"=>$classesID,"mark.subjectID"=>$subjectID,"student.sectionID"=>$sectionID,"mark.mark !="=>"n"));
		$this->db->join('student', 'student.studentID = mark.studentID', 'LEFT');
		$query = $this->db->get();
		return $query->result();
	}
	...
	function get_ssubject_average_end_term($examID,$subjectID,$classesID,$sectionID,$term,$year) {
		$this->db->select('avg((mark.mark/mark.out_of)*100) as mark')->from('mark');
		$this->db->where(array("mark.classesID"=>$classesID,"mark.subjectID"=>$subjectID,"student.sectionID"=>$sectionID,"mark.mark !="=>"n",'mark.term_name'=>$term,'mark.year'=>$year));
		$this->db->where("mark.examID IN (".$examID.')');
		$this->db->join('student', 'student.studentID = mark.studentID', 'LEFT');
		$query = $this->db->get();
		return $query->result();
	...
	function get_student_average($studentID,$examID,$classesID) {
		$query=$this->db->select('avg((mark/out_of)*100) as avg')->from('mark')->where(array('examID'=>$examID,'classesID'=>$classesID,'studentID'=>$studentID,'mark !='=>'n'));
		$query = $this->db->get();
		return $query->result();
	}
	function get_students_average($examID,$classesID,$year,$term) {
		$this->db->select('student.kcpe_mark,student.sectionID,student.photo,student.name,student.roll, student.studentID,avg(score) as avg, sum(score) as stotal')->from('processed_mark')->where(array('processed_mark.examID'=>$examID,'processed_mark.term_name'=>$term,'processed_mark.year'=>$year,'processed_mark.classesID'=>$classesID,'processed_mark.score !='=>'n'))->group_by('processed_mark.studentID')->order_by('stotal desc');
		$this->db->join('student', 'student.studentID = processed_mark.studentID', 'LEFT');
		$query = $this->db->get();
		return $query->result();
	}
	...
	function get_students_average_end_term($examID,$classesID,$year,$term) {
		$query=$this->db->select('student.kcpe_mark,student.sectionID,student.photo,student.name,student.roll, student.studentID, avg((mark.mark/mark.out_of)*100) as avg,sum((mark.mark/mark.out_of)*100) as stotal')->from('mark');
		$this->db->where(array('mark.classesID'=>$classesID,'mark.mark !='=>'n','mark.year'=>$year,'mark.term_name'=>$term));
		$this->db->where('mark.examID IN ('.$examID.')');
		$this->db->group_by('mark.studentID');
		$this->db->order_by('stotal desc');
		$this->db->join('student', 'student.studentID = mark.studentID', 'LEFT');
		$query = $this->db->get();
		return $query->result();
	}
	...
	function get_studentss_average($examID,$classesID,$year,$term) {
		$query=$this->db->select('student.name,student.roll, student.studentID, sum(score) as stotal,processed_mark.term_name as term')->from('processed_mark')->where(array('processed_mark.examID'=>$examID,'processed_mark.classesID'=>$classesID,'processed_mark.score !='=>'n'))->group_by('processed_mark.studentID');
		$this->db->join('student', 'student.studentID = processed_mark.studentID', 'LEFT');
		$query = $this->db->get();
		return $query->result();
	}
	...
	function get_studentss_average_end_term($examID,$classesID,$year,$term) {
		$query=$this->db->select('student.name,student.roll, student.studentID, sum((mark.mark/out_of)*100) as stotal,mark.term_name as term')->from('mark');
		$query=$this->db->where(array('mark.classesID'=>$classesID,'mark.mark !='=>'n','mark.year'=>$year,'mark.term_name'=>$term));
		$query=$this->db->where('mark.examID IN ('.$examID.')');
		$query=$this->db->group_by('mark.studentID');
		$this->db->join('student', 'student.studentID = mark.studentID', 'LEFT');
		$query = $this->db->get();
		return $query->result();
	}
	...
	public function get_overall_position($examID,$classesID,$sadmno,$year,$term){
		$i = 0; 
        $j = 0; 
        $ttmark=false;
        $rankingdetails=array();
		if ($classesID !=13 && $classesID !=14) {
			$rakingr=$this->ranking_m->get_students_average($examID,$classesID,$year,$term);
			...
			$studentsw_datas=$this->ranking_m->gets_sstudents($classesID,$examID);
		...
		if ($classesID !=13 && $classesID !=14) {
			...
			foreach($studentsw_datas as $sdatar){
	            ...
	            if ($sdatar->roll === $sadmno) {
		        	...
		        	$rankingdetails['average']=$sdatar->se;
		        	return $rankingdetails;
		        	break;
		        }
	        }
	...
	public function get_class_position($examID,$classesID,$sectionID,$sadmno,$year,$term){
		$i = 0; 
        $j = 0; 
        $ttmark=false;
        $rankingdetails=array();
		if ($classesID !=13 && $classesID !=14) {
			$rakingr=$this->ranking_m->get_sstudent($classesID,$sectionID,$examID,$year,$term);
			$totalstudents=count($rakingr);
			foreach ($rakingr as $keyvalue) {
			...
			$studentsw_datas=$this->ranking_m->get_sstudents($classesID,$sectionID,$examID);
			$totalstudents=count($studentsw_datas);
			foreach($studentsw_datas as $sdatar){
				if ($ttmark != $sdatar->tp) {
	                $ttmark=$sdatar->tp;
	...
	public function get_class_position_end_term($examID,$classesID,$sectionID,$sadmno,$year,$term){
		...
		if ($classesID !=13 && $classesID !=14) {
			...
			foreach($studentsw_datas as $sdatar){
	            ...
	            if ($sdatar->roll === $sadmno) {
	            	$rankingdetails['tp']=$sdatar->tp;
		        	$rankingdetails['position']=$i.'('.round($sdatar->tp,0).')';
		        	$rankingdetails['se']=$sdatar->se;
